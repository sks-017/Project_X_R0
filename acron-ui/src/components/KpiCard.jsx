...
export function AndonTile({ name, oee, machineCount, auxCount, assets }) {
  const state = getStatusClass(oee) || 'info'
  return (
    <div className={`andon-tile ${state}`}>
      <div className="andon-header">
        <span className="andon-name">{name}</span>
        <span className={`badge badge-${state === 'info' ? 'info' : state}`}>{state}</span>
      </div>
      <div className="andon-value">{oee != null ? `${oee.toFixed(1)}% OEE` : 'No data'}</div>
      <div className="andon-meta">
        {machineCount || 0} machines, {auxCount || 0} auxiliaries
        <br />
        {assets || ''}
      </div>
    </div>
...
export function HealthPill({ label, status }) {
  ...
  return (
    <div className="health-pill">
      ...
      {label ? `${label}: ${status || 'unknown'}` : status || 'unknown'}
    </div>
  )
}
